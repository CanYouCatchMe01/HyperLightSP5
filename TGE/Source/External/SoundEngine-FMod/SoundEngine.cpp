...
unsigned SoundEngine::PlaySound(const std::string& aFileName, const float aVolume)
{
	return myImpl->PlaySound(aFileName, aVolume);
}

unsigned SoundEngine::PlaySoundAtLocation(const std::string& aFileName, const float aVolume,
...
SoundEngine::ListenerHandle SoundEngine::GetNextFreeListener()
{
	...
	if(numListeners < 8)
	{
		...
		assert(myImpl->AddListener(newListener.Location, newListener.Velocity, newListener.Forward, newListener.Up));
...
bool SoundEngine::SetListenerPosition(ListenerHandle aHandle, std::array<float, 3> aPosition)
{
	if(aHandle.IsValid())
	{
		...
		{
			return myImpl->SetListenerTransform(listener.FmodId, aPosition, vel, fwd, up);
		}
	}
